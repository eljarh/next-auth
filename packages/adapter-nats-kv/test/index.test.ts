...
//This function allows us to open a connection when we don't care to close it
async function getNextAuthKV(): Promise<KV> {
  const nc = await connect({
    servers: "nats://localhost:5222",
    authenticator: undefined,
  })
  const kvm = new Kvm(nc)
  return await kvm.open("authKV")
}

//wierd stuff happens if we dont await the setup
await setupAuthKV()
